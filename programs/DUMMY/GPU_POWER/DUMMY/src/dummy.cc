...
int main(int argc, char *argv[])
{
    ...
    while(!stop && !error){
        ...
        unsigned int smCount = vm["sm"].as<unsigned int>();
        unsigned int warpSize = vm["threads"].as<unsigned int>();
...
po::options_description SetupOptions()
{
    ...
    ("sm,N", po::value<unsigned int>()->default_value(1), "number of sm")
    ("threads,M", po::value<unsigned int>()->default_value(32), "number of threads per sm")
    ;
