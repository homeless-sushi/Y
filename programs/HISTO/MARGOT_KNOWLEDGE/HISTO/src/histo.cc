...
int kernel(unsigned int inputSize, Knobs::DEVICE device, unsigned int cpuThreads, unsigned int gpuNBlocksExp, unsigned int gpuBlockSizeExp);
            ...
            if(device == Knobs::DEVICE::CPU){
                ...
                while(true){
                    ...
                    for(unsigned int cpuThreads = cpuThreadsMin; cpuThreads <= cpuThreadsMax; cpuThreads++){
                        ...
                        kernel(inputSize, device, cpuThreads, gpuNBlocksExp, gpuBlockSizeExp);//
                        std::cout << std::endl;
                    }
